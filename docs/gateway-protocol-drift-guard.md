...
3. **Request parameters** (`RequestParameterNames_arePresentInConstructionRegion`)
   - For each `used` request method, every pinned request parameter must appear as
     an **actual constructed wire key** in the request-construction region — an
     anonymous-object member (`new { sessionKey = key, path }`) or a dictionary
     string key (`payload["model"] = …`) — not merely as a token somewhere in the
     code. So renaming a wire key while the old name survives as a local/parameter
     (`new { sessionKey = key }` when `key` is still pinned) is caught.
...
| `requestFields` of `used` request methods | **Yes** — each must be an actual constructed wire key in the region. |
