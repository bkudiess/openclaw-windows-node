...
| `src/OpenClaw.Tray.WinUI/Pages/PermissionsPage.xaml.cs` | `PermissionsPageViewModel`, capability projections, policy-store adapters |
...
| exec-approval-rule-identity | authoritative | path-only generated exec approval grants | generated durable approval hashing and `(pattern,argPattern,source)` identity | ExecArgPattern + ExecAllowlistRuleIdentity | manual path-only rules remain intentionally broad | generated Allow Always rules use the upstream length-delimited SHA-256 argv-tail contract and policy currency compares full authority tuples | ExecApprovalV2EvaluatorTests.HashedArgPattern_MatchesUpstreamLengthDelimitedVectors | behavioral | - |
...
| permissions-exec-policy-vm | authoritative | src/OpenClaw.Tray.WinUI/Pages/PermissionsPage.xaml.cs | exec approvals snapshot projection, scope selection, CAS mutation, conflict retry, and save rollback | PermissionsPageViewModel + IExecApprovalsPolicyStore | code-behind maps WinUI controls, localized copy, validation presentation, and saved/error status | valid V2 executable patterns persist to the selected scope and survive reload through the same authoritative ExecApprovalsStore | PermissionsPageViewModelTests.AddAllowlistEntry_ValidPatternPersistsToSelectedScopeAndSurvivesReload | behavioral | - |
| permissions-page-exec-policy-closed | closed | src/OpenClaw.Tray.WinUI/Pages/PermissionsPage.xaml.cs | direct ExecApprovalsStore reads, writes, policy mutation, and main-only allowlist ownership | PermissionsPageViewModel + IExecApprovalsPolicyStore | view event forwarding and control projection only | PermissionsPage never regains direct store or file mutation and supports defaults, wildcard, main, and existing agent scopes | AppRefactorContractTests.PermissionsPage_ExecApprovals_StaysDelegatedToViewModel | source-shape | when PermissionsPage is replaced by a declarative binding-only view |
<!-- LEDGER:END -->
