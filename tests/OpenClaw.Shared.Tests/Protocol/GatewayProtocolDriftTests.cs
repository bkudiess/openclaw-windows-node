...
public class GatewayProtocolDriftTests
{
    ...
    /// <summary>
    /// Guard C — request shapes, scoped. For each <c>used</c> request method, the
    /// pinned request parameter names must appear in the request-construction
    /// region: the explicit <c>requestFieldsSource</c> builder body when set,
    /// otherwise the enclosing block of an actual dispatch call. A field counts as
    /// present only as a bare identifier in code (masked view) or as a quoted wire
    /// key (<c>"field"</c> in the code view) — never from prose inside an unrelated
    /// string literal.
    /// </summary>
    [Fact]
    public void RequestParameterNames_arePresentInConstructionRegion()
    {
        ...
        foreach (var method in snapshot.Methods.Where(m => m.WindowsUsage == "used" && m.Kind == "request" && m.RequestFields.Count > 0))
        {
            ...
            foreach (var field in method.RequestFields)
            {
                if (!constructedKeys.Contains(field))
                    failures.Add($"{method.Method}.{field}: not constructed as a request wire key in the construction region (upstream rename not reconciled, or refresh the snapshot). Constructed keys: [{string.Join(", ", constructedKeys.OrderBy(s => s, StringComparer.Ordinal))}]");
            }
        }
