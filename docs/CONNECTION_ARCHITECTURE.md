...
**Security — trust gate and endpoint provenance.** Clearing a device token downgrades to the more powerful shared/bootstrap credential, so `IsRecoverySafeEndpoint` restricts recovery to trusted endpoints: an owned SSH tunnel, a validated TLS (`wss`/`https`) endpoint, or — for a setup-managed WSL loopback gateway — an endpoint proven by `ManagedLocalGatewayPortProvenanceService`. NAT-mode WSL requires a Microsoft-signed Windows WSL relay plus the expected distro's systemd MainPID owning the guest socket. Mirrored networking has no Windows relay/listener row, so it is accepted only when `wslinfo` reports `mirrored`, the expected MainPID owns every loopback family the saved host can resolve to, and Windows rejects exclusive binds on those addresses. The proof is repeated immediately before strong-credential handoff. Loopback is not treated as identity by itself: an unknown listener or a proven obsolete native OpenClaw gateway blocks fallback, so a wrong local process cannot return a device-token mismatch to induce disclosure of the shared credential. A plain `ws://` remote endpoint is never eligible.
