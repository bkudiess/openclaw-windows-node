...
`{ path, exists, hash, baseHash, file: { version, defaults: { security, ask, askFallback, autoAllowSkills }, agents: { "<agentId>": { security, ask, askFallback, autoAllowSkills, allowlist: [{ id, pattern, source?, argPattern?, lastUsedAt?, lastResolvedPath? }] } } } }`.
...
Rules generated by **Always Allow** use `source: "allow-always"` plus
`argPattern: "sha256:argv:<digest>"`; they match the executable path and the
exact approved argument tail. A legacy generated rule with `source` but no
`argPattern` is ignored. A hand-authored rule with neither field remains an
intentionally broad executable-path rule.
...
allowlist grants, but it cannot add or change a rule's path, `source`, or
`argPattern`, or set full access. Missing or
